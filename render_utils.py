...
def batchify_rays(t, chain_5frames,
                rays_flat, chunk=1024*16, ref=None, **kwargs):
    """Render rays in smaller minibatches to avoid OOM.
    """
    all_ret = {}
    for i in range(0, rays_flat.shape[0], chunk):
        ret = render_rays(t, chain_5frames, rays_flat[i:i+chunk], ref, **kwargs)
        for k in ret:
            if k not in all_ret:
                all_ret[k] = []
...
def render_rays(t,
                ...
                network_fn_d_pixel,
                network_fn_s_pixel,
                network_query_fn_d_pixel,
                network_query_fn_s_pixel,
                N_samples,
                num_img,
                DyNeRF_blending,
                pretrain=False,
    ...
    raw_s = network_query_fn_s_pixel(pts_ref[..., :3], viewdirs, network_fn_s_pixel, f_projection)
    ...
    raw_d = network_query_fn_d_pixel(pts_ref, viewdirs, network_fn_d_pixel, f_projection)
    ...
    with torch.no_grad():
        pts_b_feature = ref.feature_matching(pts_b[..., :3])
    raw_d_b = network_query_fn_d_pixel(pts_b, viewdirs, network_fn_d_pixel, pts_b_feature)
    ...
    with torch.no_grad():
        pts_f_feature = ref.feature_matching(pts_f[..., :3])
    raw_d_f = network_query_fn_d_pixel(pts_f, viewdirs, network_fn_d_pixel, pts_f_feature)
    ...
    ret['raw_d_f_hat'] = torch.sum(weights_d_f[..., None] * pts_f[..., :3], -2) #xi hat t+1
    ...
    if chain_5frames:
        ...
        with torch.no_grad():
            pts_b_b_feature = ref.feature_matching(pts_b_b[..., :3])
        raw_d_b_b = network_query_fn_d_pixel(pts_b_b, viewdirs, network_fn_d_pixel, pts_b_b_feature)
    ...
    if chain_5frames:
        ...
        with torch.no_grad():
            pts_f_f_feature = ref.feature_matching(pts_f_f[..., :3])
        raw_d_f_f = network_query_fn_d_pixel(pts_f_f, viewdirs, network_fn_d_pixel, pts_f_f_feature)
