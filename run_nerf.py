...
def config_parser():
    ...
    parser.add_argument('--config', is_config_file=True,
                        default="./configs/config_Balloon2.txt",
    ...
    parser.add_argument("--lrate", type=float, default=5e-4,
                        help='learning rate')
    parser.add_argument("--d_lrate", type=float, default=5e-4,
                        help='learning rate')
...
def train():
    ...
    if args.render_only:
        ...
        target_d = images[img_i_d]
        target_f = images[img_i_f]
        

        if img_i == len(i_train) - 1:
            rgb = torch.cat([target.unsqueeze(0), target_d.unsqueeze(0)], dim=0)
            ref_pose = torch.cat([poses[img_i: img_i + 1, :3, :4], poses[img_i_d: img_i_d + 1, :3, :4]], dim=0)
        elif img_i == 0:
            rgb = torch.cat([target.unsqueeze(0), target_f.unsqueeze(0)], dim=0)
            ref_pose = torch.cat([poses[img_i: img_i + 1, :3, :4], poses[img_i_f: img_i_f + 1, :3, :4]], dim=0)
        else:
            rgb = torch.cat([target.unsqueeze(0), target_f.unsqueeze(0), target_d.unsqueeze(0)], dim=0)
            ref_pose = torch.cat([poses[img_i: img_i + 1, :3, :4], poses[img_i_f: img_i_f + 1, :3, :4], poses[img_i_d: img_i_d + 1, :3, :4]], dim=0)

        with torch.no_grad():
            reference_feature = encoder(rgb.permute(0, 3, 1, 2))

        ref = Reference(reference_feature, ref_pose, focal, target.shape[1])
        ...
        with torch.no_grad():
            ret = render_path(pose2render, time2render,
                              hwf, args.chunk, render_kwargs_test, savedir=testsavedir, ref=ref)
    ...
    if args.pretrain:
        ...
        if args.ft_path_S is not None and args.ft_path_S != 'None':
            ...
            render_kwargs_train['network_fn_s_pixel'].load_state_dict(ckpt['network_fn_s_pixel_state_dict'])
        else:
        ...
        torch.save({
            ...
            'network_fn_s_pixel_state_dict': render_kwargs_train['network_fn_s_pixel'].state_dict(),
        ...
        grad_vars_d = list(render_kwargs_train['network_fn_d_pixel'].parameters())
        optimizer = torch.optim.Adam(params=grad_vars_d, lr=args.d_lrate, betas=(0.9, 0.999))
    ...
    for i in range(start, args.N_iters):
        ...
        rays_o = select_batch(rays_o) # (N_rand, 3)
        rays_d = select_batch(rays_d) # (N_rand, 3)
        target_rgb = select_batch(target) #(270 ,480 ,3 ,12)
        ...
        slow_loss = L1(ret['sceneflow_b']) + L1(ret['sceneflow_f']) + L1(ret['sceneflow_b'] + ret['sceneflow_f'])
        ...
        decay_rate = 0.1
        decay_steps = args.lrate_decay
        new_lrate = args.d_lrate * (decay_rate ** (global_step / decay_steps))
        for param_group in optimizer.param_groups:
            param_group['lr'] = new_lrate
        ...
        if i % args.i_weights==0:
            ...
            if args.N_importance > 0:
                ...
                torch.save({
                    ...
                    'network_fn_d_pixel_state_dict': render_kwargs_train['network_fn_d_pixel'].state_dict(),
                    # 'network_fn_s_state_dict': render_kwargs_train['network_fn_s'].state_dict(),
                    'network_fn_s_pixel_state_dict': render_kwargs_train['network_fn_s_pixel'].state_dict(),
                    'optimizer_state_dict': optimizer.state_dict(),
                }, path)
        ...
        if i % args.i_testset == 0 and i > 0:
            ...
            if img_i == len(i_train) - 1:
                ...
                rgb = torch.cat([target.unsqueeze(0), target_f.unsqueeze(0), target_d.unsqueeze(0)], dim=0)
                ref_pose = torch.cat([poses[img_i: img_i + 1, :3, :4], poses[img_i_f: img_i_f + 1, :3, :4],
                                      poses[img_i_d: img_i_d + 1, :3, :4]], dim=0)
            ...
            with torch.no_grad():
                ret = render_path(pose2render, time2render,
                                  ...
                                  flows_gt_f=grids[:, :, :, 2:4], flows_gt_b=grids[:, :, :, 5:7], ref=ref)
            moviebase = os.path.join(
                testsavedir, '{}_{}_{:06d}_'.format(expname, result_type, i))
            save_res(moviebase, ret)
        ...
        if i % args.i_img == 0:
            ...
            with torch.no_grad():
                ...
                ret['blending'] = torch.sum(ret['blending'], -1)
                writer.add_image("blending", ret['blending'], global_step=i, dataformats='HW')
